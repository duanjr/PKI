...
contract PKI{
    ...
    function register(bytes memory publicKey ,uint8 v, bytes32 r, bytes32 s) public returns(bool) {
        ...
        if (utils.verifySignature(message, v, r, s) != registeringAddress) {
            return false;
        }
    ...
    bytes memory publicKeyNew ,uint8 vNew, bytes32 rNew, bytes32 sNew) public returns(bool) {
        ...
        if (utils.verifySignature(message, vOld, rOld, sOld) != registeringAddressOld) {
            return false;
        }
        ...
        if (utils.verifySignature(message, vNew, rNew, sNew) != registeringAddressNew) {
            return false;
        }
